function draw_window_properties() {
	...
	a = cursong.loop
	if (language != 1) {if (draw_checkbox(x1 + 232, y1 + 285, cursong.loop, "Enable looping", "Whether to loop this song back to"+br+"the start at the end of playback.", false, true)) cursong.loop=!cursong.loop}
	else {if (draw_checkbox(x1 + 232, y1 + 285, cursong.loop, "启用循环", "是否在播放结尾循环至开始处。", false, true)) cursong.loop=!cursong.loop}
	...
	a = cursong.loopstart
	if (cursong.loop) {
		cursong.loopstart = median(0, draw_dragvalue(7, x1 + 320, y1 + 305, cursong.loopstart, 0.5), cursong.enda)
	} else {
		draw_text_dynamic(x1 + 320, y1 + 305, cursong.loopstart)
	}
	...
	a = cursong.loopmax
	if (cursong.loop) {
		cursong.loopmax = median(0, draw_dragvalue(13, x1 + 320, y1 + 325, cursong.loopmax, 0.5), 10)
	} else {
		draw_text_dynamic(x1 + 320, y1 + 325, cursong.loopmax)
	}
	if (language != 1) {if (cursong.loopmax = 0) draw_text_dynamic(x1 + 340, y1 + 325, "(infinite)")}
	else {if (cursong.loopmax = 0) draw_text_dynamic(x1 + 340, y1 + 325, "（无限）")}
