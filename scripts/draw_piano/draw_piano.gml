    ...
    if (a > 0 && c != 2 && c != 5) { // Sharp key to the left
        ...
        if (window = 0 && mouse_rectangle(xx + 39 * a, yy, 39, 128)) {
            if (!mouse_rectangle(xx + 39 * a - 12, yy - 7, 25, 71) && (!mouse_rectangle(xx + 39 * a + 27, yy - 7, 25, 71) || (a = k - 1 || c = 1 || c = 4))) {
                ...
                if (t = 0 && key_click[c2]) {
                    selectedkey = c2
                    play_sound(instrument, c2, 1,100)
                }
                if (a = k - 1) d = 1
                if (a = 0) d = 0
            } else {
        ...
        if (window = 0 && mouse_rectangle(xx + 39 * a - 12, yy - 7, 25, 71)) {
            ...
            if (t = 0 && key_click[c1]) {
                selectedkey = c1
                play_sound(instrument, c1, 1,100)
            }
        } else {
            key_click[c1] = 0
        }
...
for (a = 0 ;a < 87; a += 1) {
    if (piano_key[a] > 0) {
        if (key_press[a] = 0 && keyboard_check(piano_key[a])) {
            ...
            if (playing = 0) play_sound(instrument, a, 1,100)
            if (playing = 0.25) toggle_playing(totalcols)
            if (playing && record) {
                b = 0
                while (!add_block_manual(ceil(marker_pos), b, instrument, a)) b += 1
