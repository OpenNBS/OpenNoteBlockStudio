function draw_checkbox() {
	...
	if (lock = 1) {
		...
		if (is_switch) {
			...
			if (expression) draw_sprite_ext(spr_switch, m + 3, xx, yy, 1, 1, 0, accent[3 + 2 * (m = 1) + 5 * (m = 2)], draw_get_alpha())
		} else {
			draw_sprite(spr_checkbox, m + 6 * theme + 6 * (fdark) * (theme = 3), xx, yy)
			if (expression) draw_sprite_ext(spr_checkbox, 3 + 6 * theme, xx, yy, 1, 1, 0, accent[3 + 2 * (m = 1) + 5 * (m = 2)] * (theme = 3) - !(theme = 3), draw_get_alpha())
		}
		draw_text(xx + x_offset, yy - 1, str)
