function open_midi(argument0) {
	...
	if (fn = "") {
		if (language != 1) fn = string(GetOpenFileName("MIDI Sequences (*.mid)|*.midi;*.mid", "", "", "Import from MIDI"))
		else fn = string(GetOpenFileName("MIDI Sequences (*.mid)|*.midi;*.mid", "", "", "从 MIDI 导入"))
	}
