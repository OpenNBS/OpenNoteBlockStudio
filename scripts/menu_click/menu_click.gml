...
switch (m) {
    ...
    case "edit": {
        ...
        if (sel = 21 + ds_list_size(instrument_list)) macro_stereoize()
		if (sel = 22 + ds_list_size(instrument_list)) macro_glissando_up()
		if (sel = 23 + ds_list_size(instrument_list)) macro_glissando_down()
        if (sel = 24 + ds_list_size(instrument_list)) macro_volumelfo()
        if (sel = 25 + ds_list_size(instrument_list)) macro_fadein()
        if (sel = 26 + ds_list_size(instrument_list)) macro_fadeout()
        if (sel = 27 + ds_list_size(instrument_list)) {
